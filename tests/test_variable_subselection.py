...
class TestVariableSubselection(basetest.CDMSBaseTest):
    def setUp(self):
        ...
        self.xp = numpy.ma.concatenate(
            (self.test_arr[1, 1:, 4:12, 8:self.NLON], self.test_arr[1, 1:, 4:12, 0:8]), axis=2)

    def testGetRegion(self):
        # Positional
        s = self.var.getRegion(
            (366., 731., 'ccn'), (-42., 42., 'ccn'), (90., 270., 'con'))
        self.assertTrue(numpy.ma.allequal(self.vp, s))
        # Keyword
        s = self.var.getRegion(latitude=(-42., 42., 'ccn'),
                               longitude=(90., 270., 'con'), time=(366., 731., 'ccn'))
        self.assertTrue(numpy.ma.allequal(self.vp, s))
        # Wraparound
        s = self.var.getRegion(time=(
            366., 731., 'ccn'), latitude=(-42., 42., 'ccn'), longitude=(90., 450., 'con'))
    ...
    def testSubRegion(self):
        # positional
        s2 = self.var.subRegion(
            (366., 731., 'ccn'), (-42., 42., 'ccn'), (90., 270., 'con'))
        self.assertTrue(numpy.ma.allequal(self.vp, s2))

        # squeeze
        s2 = self.var.subRegion(
            (731., 731., 'ccn'), (-42., 42., 'ccn'), (90., 270., 'con'), squeeze=1)
    ...
    def testOrder(self):
        ...
        s2 = self.var.subRegion(latitude=(-42.,
                                          42.,
                                          'ccn'),
                                longitude=(90.,
                                           270.,
    ...
    def testSubRegionTimes(self):
        ...
        s2 = self.var.subRegion(
            latitude=(-42., 42., 'ccn'), longitude=(90., 270., 'con'), time=(t1, t2))
        self.assertTrue(numpy.ma.allequal(self.vp, s2))
        ...
        s2 = self.var.subRegion(latitude=(-42., 42., 'ccn'),
                                longitude=(90., 270., 'con'), time=(t1, t2, 'ccn'))
        self.assertTrue(numpy.ma.allequal(self.vp, s2))
...
class TestCDTime(basetest.CDMSBaseTest):
    ...
    def cmpYear(self, c1, c2):
        return ((c1.year == c2.year) and (c1.month == c2.month) and (c1.day == c2.day)) or (
            (c1.year == c2.year == 1582) and (c1.month == c2.month == 10) and (c1.day in [5, 15] and (c2.day in [5, 15])))
    ...
    def testCalendars(self):
        ...
        self.doCalTest(1970, 1, 1, 1583, 1, 1, -
                       141349.00, cdtime.GregorianCalendar)
