...
class Test(unittest.TestCase):
    ...
    def test0_ESMF(self):
        ...
        srcBounds = _getCorners(
            [srcF.variables['lat_vertices'][:], srcF.variables['lon_vertices'][:]])
        ...
        lat2dBounds = numpy.outer(
            lat1dBounds, numpy.ones(
                (len(lon1dBounds),), lon1dBounds.dtype))
        lon2dBounds = numpy.outer(
            numpy.ones(
                (len(lat1dBounds),), lat1dBounds.dtype), lon1dBounds)
        lat2dCENTER = numpy.outer(
            lat1dCENTER, numpy.ones(
                (len(lon1dCENTER),), lon1dCENTER.dtype))
        lon2dCENTER = numpy.outer(
            numpy.ones(
                (len(lat1dCENTER),), lat1dCENTER.dtype), lon1dCENTER)
        ...
        # set the coordinates and field values. In ESMF, arrays are column
        # major!
