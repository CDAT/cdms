...
class TestESMPRegridderConserve(unittest.TestCase):
    ...
    def test1_3D_Native_Bilinear(self):
        ...
        dstGrid3D = ESMF.Grid(maxIndex, coord_sys = ESMF.CoordSys.CART, staggerloc=[ESMF.StaggerLoc.CENTER])
        maxIndex = numpy.array(srcDims, dtype = numpy.int32)
        srcGrid3D = ESMF.Grid(maxIndex, coord_sys = ESMF.CoordSys.CART, staggerloc=[ESMF.StaggerLoc.CENTER])
    ...
    def xtest2_3D_Native_Conserve(self):
        ...
        maxIndex = numpy.array(dstDims, dtype = numpy.int32)
        dstGrid3D = ESMF.Grid(maxIndex, num_peri_dims=0, coord_sys = ESMF.CoordSys.CART, staggerloc=[ESMF.StaggerLoc.CENTER])
        maxIndex = numpy.array(srcDims, dtype = numpy.int32)
        srcGrid3D = ESMF.Grid(maxIndex, num_peri_dims=0, coord_sys = ESMF.CoordSys.CART, staggerloc=[ESMF.StaggerLoc.CENTER])
