...
class TestForecastIO(basetest.CDMSBaseTest):
    def testForecast(self):
        ...
        taxis.designateTime()
        v1 = cdms2.tvariable.TransientVariable(
            [[1.1, 1.2], [2.1, 2.1]], axes=[taxis, xaxis], id='var')
        v2 = cdms2.tvariable.TransientVariable(
            [[3.1, 3.2], [4.1, 4.1]], axes=[taxis, xaxis], id='var')
        v3 = cdms2.tvariable.TransientVariable(
            [[5.1, 5.2], [6.1, 6.1]], axes=[taxis, xaxis], id='var')
        ...
        cdscan(
            "cdscan -q --forecast -x test_fc.xml test_fc1 test_fc2 test_fc3".split())
        # Read in the data.

        fcs = cdms2.forecast.forecasts(
            'test_fc.xml', ("2010-08-25", "2010-08-27"))
        vin = fcs('var')
