...
class TestESMFRegridderConserve(unittest.TestCase):
    ...
    def test_3D_Native(self):
        ...
        dstGrid3D = ESMF.Grid(maxIndex, num_peri_dims=0, staggerloc=[ESMF.StaggerLoc.CENTER_VCENTER], 
                              coord_sys=ESMF.CoordSys.CART)
#        dstGrid3D = ESMF.ESMP_GridCreateNoPeriDim(maxIndex, 
#                                         coordSys = ESMP.ESMP_COORDSYS_CART)

        # Destination Corners
        dstGrid3D.add_coords([ESMF.StaggerLoc.CORNER_VFACE])
        #ESMP.ESMP_GridAddCoord(dstGrid3D, 
        #                    staggerloc = ESMP.ESMP_STAGGERLOC_CORNER_VFACE)

        dstDimsCorner = [dstGrid3D.lower_bounds[ESMF.StaggerLoc.CORNER_VFACE], 
                         dstGrid3D.upper_bounds[ESMF.StaggerLoc.CORNER_VFACE]]
        #dstDimsCorner = ESMP.ESMP_GridGetCoord(dstGrid3D, 
        #                    staggerloc = ESMP.ESMP_STAGGERLOC_CORNER_VFACE)
        dstXCorner = dstGrid3D.get_coords(coord_dim=0,staggerloc=ESMF.StaggerLoc.CORNER_VFACE)
        #dstXCorner = ESMP.ESMP_GridGetCoordPtr(dstGrid3D, 0,
        #                    staggerloc = ESMP.ESMP_STAGGERLOC_CORNER_VFACE)
        dstYCorner = dstGrid3D.get_coords(coord_dim=1,staggerloc=ESMF.StaggerLoc.CORNER_VFACE)
        #dstYCorner = ESMP.ESMP_GridGetCoordPtr(dstGrid3D, 1,
        #                    staggerloc = ESMP.ESMP_STAGGERLOC_CORNER_VFACE)
        dstZCorner = dstGrid3D.get_coords(coord_dim=2,staggerloc=ESMF.StaggerLoc.CORNER_VFACE)
        ...
        dstDimsCenter = [dstGrid3D.lower_bounds[ESMF.StaggerLoc.CENTER_VCENTER], 
                         dstGrid3D.upper_bounds[ESMF.StaggerLoc.CENTER_VCENTER]]
#        dstDimsCenter = ESMP.ESMP_GridGetCoord(dstGrid3D, 
#                            staggerloc = ESMP.ESMP_STAGGERLOC_CENTER_VCENTER)
        dstXCenter = dstGrid3D.get_coords(coord_dim=0, staggerloc=ESMF.StaggerLoc.CENTER_VCENTER)
        dstYCenter = dstGrid3D.get_coords(coord_dim=1, staggerloc=ESMF.StaggerLoc.CENTER_VCENTER)
        dstZCenter = dstGrid3D.get_coords(coord_dim=2, staggerloc=ESMF.StaggerLoc.CENTER_VCENTER)
...
#                            staggerloc = ESMP.ESMP_STAGGERLOC_CENTER_VCENTER)
        ...
        dstField = ESMF.Field(grid=dstGrid3D, name='dst_salinity',
...
#        dstFieldPtr = ESMP.ESMP_FieldGetPtr(dstField)
        ...
        srcGrid3D = ESMF.Grid(maxIndex, num_peri_dims=0, staggerloc=[ESMF.StaggerLoc.CENTER_VCENTER],
                              coord_sys=ESMF.CoordSys.CART)

#        srcGrid3D = ESMP.ESMP_GridCreateNoPeriDim(maxIndex, 
#                                         coordSys = ESMP.ESMP_COORDSYS_CART)

        srcGrid3D.add_coords([ESMF.StaggerLoc.CORNER_VFACE])
        #ESMP.ESMP_GridAddCoord(dstGrid3D, 
        #                    staggerloc = ESMP.ESMP_STAGGERLOC_CORNER_VFACE)

        srcDimsCorner = [srcGrid3D.lower_bounds[ESMF.StaggerLoc.CORNER_VFACE], 
                         srcGrid3D.upper_bounds[ESMF.StaggerLoc.CORNER_VFACE]]
        # Source Corners
...
#                            staggerloc = ESMP.ESMP_STAGGERLOC_CORNER_VFACE)
        srcXCorner = srcGrid3D.get_coords(coord_dim=0, staggerloc=ESMF.StaggerLoc.CORNER_VFACE)
        srcYCorner = srcGrid3D.get_coords(coord_dim=1, staggerloc=ESMF.StaggerLoc.CORNER_VFACE)
        srcZCorner = srcGrid3D.get_coords(coord_dim=2, staggerloc=ESMF.StaggerLoc.CORNER_VFACE)
...
#                            staggerloc = ESMP.ESMP_STAGGERLOC_CORNER_VFACE)
        ...
        srcXCorner[:] = srcXYZCorner[0][srcKCornerBE, 
                                                  srcJCornerBE, 
                                                  srcICornerBE].T
        srcYCorner[:] = srcXYZCorner[1][srcKCornerBE,
                                                  srcJCornerBE, 
                                                  srcICornerBE].T
        srcZCorner[:] = srcXYZCorner[2][srcKCornerBE,
                                                  srcJCornerBE, 
                                                  srcICornerBE].T
        # Source Centers
        srcDimsCenter = [srcGrid3D.lower_bounds[ESMF.StaggerLoc.CENTER_VCENTER],   
                         srcGrid3D.upper_bounds[ESMF.StaggerLoc.CENTER_VCENTER]]

        # Source Corners
        srcXCenter = srcGrid3D.get_coords(coord_dim=0, staggerloc=ESMF.StaggerLoc.CENTER_VCENTER)
        srcYCenter = srcGrid3D.get_coords(coord_dim=1, staggerloc=ESMF.StaggerLoc.CENTER_VCENTER)
        srcZCenter = srcGrid3D.get_coords(coord_dim=2, staggerloc=ESMF.StaggerLoc.CENTER_VCENTER)
...
#                            staggerloc = ESMP.ESMP_STAGGERLOC_CENTER_VCENTER)
        ...
        srcXCenter[:] = srcXYZCenter[0][srcKCenterBE, 
                                                  srcJCenterBE, 
                                                  srcICenterBE].T
        srcYCenter[:] = srcXYZCenter[1][srcKCenterBE,
                                                  srcJCenterBE, 
                                                  srcICenterBE].T
        srcZCenter[:] = srcXYZCenter[2][srcKCenterBE,
                                                  srcJCenterBE, 
                                                  srcICenterBE].T
        # Source Mask
        pdb.set_trace()
...
#                                           item = ESMP.ESMP_GRIDITEM_MASK)
        srcMaskPtr[:] = so[srcKCornerBE, srcJCornerBE, srcICornerBE].mask.T

        srcField = ESMF.Field(grid=srcGrid3D, name='dst_salinity',
                                  staggerloc = ESMF.StaggerLoc.CENTER_VCENTER,
                                  typekind = ESMF.TypeKind.R4)
        srcFieldPtr = srcField.data
        # Create and Populate Source Field
        aa = numpy.array(so)
        srcFieldPtr[:] = aa[srcKCornerBE, srcJCornerBE, srcICornerBE].T
        ...
        regridBck = ESMF.Regrid(dstField, srcField,
