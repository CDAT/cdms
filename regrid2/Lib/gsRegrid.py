...
def getTensorProduct(axis, dim, dims):
    ...
    ----------
         ...
         axis 1D array of coordinates

         dim dimensional index of the above coordinate

         dims sizes of all coordinates
...
def makeCoordsCyclic(coords, dims):
    ...
    -------
         ...
         new, extended coordinates such that the longitudes cover the sphere
            and new dimensions
    """
...
def checkForCoordCut(coords, dims):
    ...
    inTopRow = False
    if len(maxLats[0] > 0):
        inTopRow = numpy.all(maxLats[-2] == nlat - 1)
    if not inTopRow:
    ...
    if nextIndex != index + 1 and nextIndex != nlon - 1:
        minCount += 1
    if minCount == 1:
...
def handleCoordsCut(coords, dims, bounds):
    ...
    ----------
         ...
         coords input coordinates list of rank

         dims input dimensions

         bounds boundaries for each coordinate

    Returns
    -------

         extended coordinates such that there is an extra row containing
    ...
    def getIndices(array, nlon, newI):
        ...
        ----------
             ...
             newI index row with connectivity to be updated
...
class Regrid:
    ...
    def __init__(self, src_grid, dst_grid, src_bounds=None, mkCyclic=False,
        ...
        ----------
             ...
             src_grid source grid, a list of [x, y, ...] coordinates
                        or a cdms2.grid.Transient

             dst_grid destination grid, a list of [x, y, ...] coordinates

             src_bounds list of cell bounding coordinates (to be used when
                          handling a cut in coordinates)

             mkCyclic Add a column to the right side of the grid to complete
               a cyclic grid

             handleCut Add a row to the top of grid to handle a cut for
...
#                    pass
    ...
    def setValidMask(self, inMask):
        ...
        ----------
             ...
             inMask flat numpy array of type numpy.int32 or a valid cdms2 variable
                    with its mask set.
    ...
    def computeWeights(self, nitermax=100, tolpos=1.e-2):
        ...
        ----------
             ...
             tolpos max tolerance when locating destination positions in
               index space
        """
    ...
    def apply(self, src_data_in, dst_data, missingValue=None):
        ...
        ----------
             ...
             src_data data on source grid

             dst_data data on destination grid

             missingValue value that should be set for points falling outside the src domain,
        ...
        if src_data.dtype == numpy.float64:
            ...
            status = self.lib.nccf_set_data_double(
                src_dataid, ptr, save, fill_value)
            catchError(status, sys._getframe().f_lineno)
            ...
            status = self.lib.nccf_set_data_float(
                src_dataid, ptr, save, fill_value)
            catchError(status, sys._getframe().f_lineno)
        else:
        ...
        if dst_data.dtype == numpy.float64:
            ...
            status = self.lib.nccf_set_data_double(
                dst_dataid, ptr, save, fill_value)
            catchError(status, sys._getframe().f_lineno)
        ...
        # Now apply weights
        status = self.lib.nccf_apply_regrid(
            self.regridid, src_dataid, dst_dataid)
        catchError(status, sys._getframe().f_lineno)
    ...
    def getIndicesAndWeights(self, dst_indices):
        ...
        ----------
             ...
             dst_indices index set on the target grid
    ...
    def _findIndices(self, targetPos, nitermax, tolpos,
        ...
        ----------
             ...
             targetPos numpy array of target positions
   
             nitermax max number of iterations

             tolpos max toelrance in positions

             dindicesGuess guess for the floating point indices

        Returns
        -------

             indices, number of iterations, achieved tolerance
        """
        ...
        coord_periodicity = float(
            'inf') * numpy.ones((self.rank), targetPos.dtype)
        coord_periodicity_ptr = coord_periodicity.ctypes.data_as(
            POINTER(c_double))
